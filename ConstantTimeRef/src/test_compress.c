...
{
  int i, passed = 0;
  int allowed_v = KYBER_Q / (1 << (p->dv+1)) + 1;
  int allowed_u = KYBER_Q / (1 << (p->du+1)) + 1;
  ...
  for(i=0;i<p->n;i++) comp[i] = compress_q((int16_t)((i*137+23)%KYBER_Q), p->dv);
  ...
  max_err=0;
  for(i=0;i<p->n;i++){
    int16_t orig=(int16_t)((i*137+23)%KYBER_Q);
  ...
  for(i=0;i<p->n;i++) comp[i]=compress_q((int16_t)((i*211+57)%KYBER_Q),p->du);
  memset(packed,0,sizeof(packed));
  ...
  max_err=0;
  for(i=0;i<p->n;i++){
    int16_t orig=(int16_t)((i*211+57)%KYBER_Q);
  ...
  { int ok=1;
    ...
    int32_t err=(int32_t)(KYBER_Q-1)-(int32_t)decompress_q(compress_q(KYBER_Q-1,p->dv),p->dv);
    if(err<0)err=-err; if(err>KYBER_Q/2)err=KYBER_Q-err;
  ...
  { int ok=1;
    for(i=0;i<p->n;i++){
      int16_t orig=(int16_t)((i*97+31)%KYBER_Q);
...
{
  poly orig, recov;
  uint8_t comp[KYBER_POLYCOMPRESSEDBYTES];
  ...
  int dv=4;
#endif
  int allowed=KYBER_Q/(1<<(dv+1))+1;

  printf("\n--- Actual poly_compress/poly_decompress (MODE=%d, N=%d, dv=%d) ---\n",
         WEAVER_MODE, KYBER_N, dv);

  for(i=0;i<KYBER_N;i++) orig.coeffs[i]=(int16_t)((i*137+23)%KYBER_Q);
  poly_compress(comp, &orig);

  printf("  Compressed bytes (first 8): ");
  for(i=0;i<8&&i<KYBER_POLYCOMPRESSEDBYTES;i++) printf("%02X ",comp[i]);
  printf("\n");
...
int main(void)
{
  ...
  compress_param sets[3]={
    {"WEAVER-512",  128, 4, 9,  9 },
    {"WEAVER-1024", 256, 4, 10, 10},
    {"WEAVER-2048", 512, 6, 10, 10}
  };
