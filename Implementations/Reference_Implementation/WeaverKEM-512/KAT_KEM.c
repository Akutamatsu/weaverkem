...
/****************output KAT_KEM_WeaverKEM-512.txt****************/
int main()
{
	unsigned char *nonce;
	DRNG_ctx drng_seed;
	...
	if (0 != create_directory(dir_name)) { fprintf(stderr, "ERROR: Generate folder \"%s\" failed at %s, line %d. \n", dir_name, __FILE__, __LINE__); return KAT_FILE_OPERATE_FAILED; }
	file_output = fopen(file_path, "wb");
	if (NULL == file_output) { fprintf(stderr, "ERROR: Generate \"%s\" failed at %s, line %d. \n", algoname_output, __FILE__, __LINE__); return KAT_FILE_OPERATE_FAILED; }
	...
	for (int i = 0; i < 10; i++)
	{
		...
		if (rtn) { fprintf(stderr, "ERROR: kem_keygen returned %d at %s, line %d. \n", rtn, __FILE__, __LINE__); return KAT_KEM_CRYPTO_FAILURE; }
		...
		if (rtn) { fprintf(stderr, "ERROR: kem_enc returned %d at %s, line %d. \n", rtn, __FILE__, __LINE__); return KAT_KEM_CRYPTO_FAILURE; }
		...
		if (rtn) { fprintf(stderr, "ERROR: kem_dec returned %d at %s, line %d. \n", rtn, __FILE__, __LINE__); return KAT_KEM_CRYPTO_FAILURE; }
		if (memcmp(ss, ss1, ss_len_bytes)) { fprintf(stderr, "ERROR: SS mismatch at %s, line %d. \n", __FILE__, __LINE__); return KAT_KEM_SS_UNEQUAL; }
		fprintf(file_output, "\n");
	}
	free(seed); free(ct); free(ss1); free(ss); free(sk); free(pk); free(nonce);
	if (0 != fclose(file_output)) { fprintf(stderr, "ERROR: Close failed at %s, line %d. \n", __FILE__, __LINE__); return KAT_FILE_OPERATE_FAILED; }
...
{
	int rt = 0; char c = '\0';
	if (strlen(algorithm) > 64) rt = KAT_ALGORITHM_INSTANCE_NAME_INVALID;
	for (int i = 0; algorithm[i] != '\0'; i++) { c = algorithm[i]; if (!(isalnum(c) || '-' == c || '_' == c)) rt = KAT_ALGORITHM_INSTANCE_NAME_INVALID; }
	return rt;
}
...
{
#if defined(_WIN32)
	if (0 == _mkdir(path)) return 0;
#else
	if (0 == mkdir(path, 0755)) return 0;
#endif
	if (errno == EEXIST) {
#if defined(_WIN32)
		if (0 == _access(path, 0)) return 0;
#else
...
static void fprintlen(FILE *file_output, char *identifier, unsigned long long len)
{
	if (OUTPUT_BLANK_TEST_VECTORS) fprintf(file_output, "%s\n", identifier);
	else fprintf(file_output, "%s%llu\n", identifier, len);
}

static void fprintstr(FILE *file_output, char *identifier, unsigned char *msg, unsigned long long len)
{
	fprintf(file_output, "%s", identifier);
	for (unsigned long long i = 0; i < len; i++) fprintf(file_output, "%02X", msg[i]);
	fprintf(file_output, "\n");
}
