...
#define reduce_avx_fq WEAVER_NAMESPACE(_reduce_avx)
extern int16_t reduce_avx_fq(int16_t *r, const int16_t *qdata);

#define reduce512_avx_fq WEAVER_NAMESPACE(_reduce512_avx)
extern int16_t reduce512_avx_fq(int16_t *r, const int16_t *qdata);

#define tomont_avx_fq WEAVER_NAMESPACE(_tomont_avx)
extern int16_t tomont_avx_fq(int16_t *r, const int16_t *qdata);

#define tomont512_avx_fq WEAVER_NAMESPACE(_tomont512_avx)
extern int16_t tomont512_avx_fq(int16_t *r, const int16_t *qdata);

#define qdata_fq WEAVER_NAMESPACE(_qdata)
...
void poly_compress(uint8_t r[WEAVER_POLYCOMPRESSEDBYTES], const poly *a)
{
  ...
  for(i=0;i<WEAVER_N/8;i++) {
    for(j=0;j<8;j++) {
      u  = a->coeffs[8*i+j];
      u += (u >> 15) & WEAVER_Q;
      t[j] = ((((uint16_t)u << 4) + WEAVER_Q/2)/WEAVER_Q) & 15;
    }
...
#else
#error "WEAVER_POLYCOMPRESSEDBYTES needs to be N*4/8, N*5/8, or N*6/8"
#endif
}
...
void poly_decompress(poly *r, const uint8_t a[WEAVER_POLYCOMPRESSEDBYTES])
{
  ...
  for(i=0;i<WEAVER_N/2;i++) {
    r->coeffs[2*i+0] = (((uint16_t)(a[0] & 15)*WEAVER_Q) + 8) >> 4;
    r->coeffs[2*i+1] = (((uint16_t)(a[0] >> 4)*WEAVER_Q) + 8) >> 4;
    a += 1;
  }
#endif
#elif (WEAVER_POLYCOMPRESSEDBYTES == (WEAVER_N * 5 / 8))
...
void poly_reduce(poly *r)
{
  ...
  for(i = 0; i < WEAVER_N; i++) {
    diff = (int32_t)r->coeffs[i] - WEAVER_Q;
    mask = (int16_t)(((uint32_t)diff | (uint32_t)(-diff)) >> 31) - 1;
    r->coeffs[i] = r->coeffs[i] + (mask & (int16_t)(-WEAVER_Q));
