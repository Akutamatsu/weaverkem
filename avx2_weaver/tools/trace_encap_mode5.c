...
static void trace_encap(const uint8_t m[KYBER_INDCPA_MSGBYTES],
                        ...
                        const uint8_t coins[KYBER_SYMBYTES])
{
  ...
  uint8_t c1[KYBER_POLYVECCOMPRESSEDBYTES];
  uint8_t c2[KYBER_POLYCOMPRESSEDBYTES];

  trace_line("coins", fnv1a_bytes(coins, KYBER_SYMBYTES));

  unpack_pk(&pkpv, seed, pk);
  trace_line("pkpv_unpack", fnv1a_polyvec(&pkpv));
  trace_line("public_seed", fnv1a_bytes(seed, KYBER_SYMBYTES));
...
int main(int argc, char **argv)
{
  ...
  uint8_t kp_coins[KYBER_SYMBYTES];
  uint8_t enc_coins[KYBER_SYMBYTES];
  uint8_t replay_coins[2 * KYBER_SYMBYTES];
  uint8_t replay_enc[KYBER_SYMBYTES];
  ...
  crypto_kem_keypair_derand(pk, sk, replay_coins);
  trace_line("replay_pk", fnv1a_bytes(pk, KYBER_PUBLICKEYBYTES));
  trace_line("replay_sk", fnv1a_bytes(sk, KYBER_SECRETKEYBYTES));
  {
    uint8_t ct[KYBER_CIPHERTEXTBYTES];
    uint8_t ss[KYBER_SSBYTES];
    uint8_t kbuf[KYBER_INDCPA_MSGBYTES + KYBER_SYMBYTES];
    uint8_t kkr[KYBER_SSBYTES + KYBER_SYMBYTES];

    memcpy(kbuf, replay_enc, KYBER_SYMBYTES);
    memset(kbuf + KYBER_SYMBYTES, 0, KYBER_INDCPA_MSGBYTES - KYBER_SYMBYTES);
    trace_line("replay_kbuf_pre_h", fnv1a_bytes(kbuf, sizeof(kbuf)));
    hash_h(kbuf + KYBER_INDCPA_MSGBYTES, pk, KYBER_PUBLICKEYBYTES);
    ...
    indcpa_enc(ct, kbuf, pk, kkr + KYBER_SSBYTES);
  ...
  trace_line("pk", fnv1a_bytes(pk, KYBER_PUBLICKEYBYTES));

  memcpy(buf, enc_coins, KYBER_SYMBYTES);
  memset(buf + KYBER_SYMBYTES, 0, KYBER_INDCPA_MSGBYTES - KYBER_SYMBYTES);
  hash_h(buf + KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES);
  shake256(kr, sizeof(kr), buf, sizeof(buf));

  trace_encap(buf, pk, kr + KYBER_SSBYTES);

  {
    uint8_t ct[KYBER_CIPHERTEXTBYTES];
    uint8_t ss[KYBER_SSBYTES];
    uint8_t enc_in[KYBER_INDCPA_MSGBYTES];

    for(i = 0; i < KYBER_SYMBYTES; i++)
      enc_in[i] = enc_coins[i];
    memset(enc_in + KYBER_SYMBYTES, 0, KYBER_INDCPA_MSGBYTES - KYBER_SYMBYTES);
    ...
    {
      uint8_t enc_short[KYBER_SYMBYTES];
      uint8_t pk2[KYBER_PUBLICKEYBYTES];
      uint8_t sk2[KYBER_SECRETKEYBYTES];
