...
def p2_cyclotomic_error_probability(ps):
    D = p2_cyclotomic_final_error_distribution(ps)
    F = abs_distribution(D)
    F = iter_law_convolution(F, ps.m)
    proba = tail_probability(F, ps.m*ps.q/4)
    return F, ps.blk*proba
...
if __name__ == "__main__":
    # Parameter sets
    ps_light = NewHopeParameterSet(512, 2, 8, 8, 12289, 2**13, 2**13, 2**3) 

    ps_paranoid = NewHopeParameterSet(1024, 4, 8, 8, 12289, 2**13, 2**13, 2**3) 

    # Analyses
    print ("NewHope512 (light):")
    summarize(ps_light)
    print ()

    print ("NewHope1024 (paranoid):")
